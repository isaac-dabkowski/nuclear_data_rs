...
impl EnergyDependentAngularDistribution {
    ...
    pub fn sample_cos_theta_at_energy(
        &self,
        energy: f64,
        unitf64: UnitF64,
    ) -> Result<f64, EnergyDependentAngularDistributionError> {
        ...
        if energy == self.energy[energy_index] {
            self.distributions[energy_index]
                .sample_cos_theta(unitf64)
                .map_err(Into::into)
        } else {
            ...
            let factor = (energy - self.energy[lower_energy_index])
                / (self.energy[upper_energy_index] - self.energy[lower_energy_index]);
...
impl<'a> std::fmt::Display for EnergyDependentAngularDistribution {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "EnergyDependentAngularDistribution({} energies)",
            self.energy.len()
        )
    }
}
...
mod tests {
    ...
    use crate::angular_distributions::{
        EquiprobableBinsAngularDistribution, IsotropicAngularDistribution,
    ...
    fn make_test_distribution() -> EnergyDependentAngularDistribution {
        ...
        let equiprobable_bins_distribution = AngularDistribution::EquiprobableBins(
            EquiprobableBinsAngularDistribution::new(Vec::from_iter(
                (0..33).map(|i| i as f64 / (33 - 1) as f64 - 1.0),
            ))
            .unwrap(),
        );
        let distributions = vec![
            ...
            equiprobable_bins_distribution,
        ];
    ...
    fn test_sample_cos_theta_at_energy_on_bounds() {
        ...
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(1.0, UnitF64(0.0));
        assert_eq!(result.unwrap(), -1.0);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(1.0, UnitF64(0.5));
        assert_eq!(result.unwrap(), 0.0);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(1.0, UnitF64(1.0));
        assert_eq!(result.unwrap(), 1.0);
        // Test tabulated distribution
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(2.0, UnitF64(0.0));
        assert_eq!(result.unwrap(), 0.0);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(2.0, UnitF64(0.5));
        assert_eq!(result.unwrap(), 0.5);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(2.0, UnitF64(1.0));
        assert_eq!(result.unwrap(), 1.0);
        // Test equiprobable bins distribution
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(3.0, UnitF64(0.0));
        assert_eq!(result.unwrap(), -1.0);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(3.0, UnitF64(0.5));
        assert_eq!(result.unwrap(), -0.5);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(3.0, UnitF64(1.0));
        assert_eq!(result.unwrap(), 0.0);
    }
    ...
    fn test_sample_cos_theta_at_energy_off_bounds() {
        ...
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(1.5, UnitF64(0.0));
        assert_eq!(result.unwrap(), -0.5);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(1.5, UnitF64(0.5));
        assert_eq!(result.unwrap(), 0.25);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(1.5, UnitF64(1.0));
        assert_eq!(result.unwrap(), 1.0);
        // Test interpolation between tabulated and equiprobable distribution
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(2.5, UnitF64(0.0));
        assert_eq!(result.unwrap(), -0.5);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(2.5, UnitF64(0.5));
        assert_eq!(result.unwrap(), 0.0);
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(2.5, UnitF64(1.0));
        assert_eq!(result.unwrap(), 0.5);
    }
    ...
    fn test_sample_cos_theta_at_energy_out_of_range() {
        let energy_dependent_angular_distribution = make_test_distribution();
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(4.0, UnitF64(0.0));
        assert!(result.is_err());
        let result =
            energy_dependent_angular_distribution.sample_cos_theta_at_energy(0.0, UnitF64(0.0));
        assert!(result.is_err());
    }
}
